...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        if value[0..8] != Png::STANDARD_HEADER {
            ...
            let mut chunk_vector: Vec<Chunk> = vec![];

            let mut counter: usize = 8;

            while counter < value.len() {
                let new_chunk = Chunk::try_from(&value[counter..]).unwrap();
                let len: usize = new_chunk.length() as usize;

                chunk_vector.push(new_chunk);
                counter += 4 + 4 + len + 4;
            }
...
impl Png {
    ...
    fn from_chunks(chunks: Vec<Chunk>) -> Png {
        let mut new_png = Png::try_from(Png::STANDARD_HEADER.as_ref()).unwrap();

        for c in chunks {
            new_png.append_chunk(c);
        }

        new_png
    }

    pub fn from_strings(chunk_type: &str, data: &str) -> Result<Png, &'static str> {
        let new_chunk = Chunk::chunk_from_strings(chunk_type, data);
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let mut out_vec: Vec<u8> = Png::STANDARD_HEADER.to_vec();

        for c in &self.chunks {
            out_vec.extend(c.as_bytes());
        }
    ...
    pub fn remove_first_chunk(&mut self, chunk_type: &str) -> Result<Chunk, &'static str> {
        for c in self.chunks.iter().enumerate() {
            if c.1.chunk_type().bytes() == chunk_type.as_bytes() {
                return Ok(self.chunks.remove(c.0))
            }
        }

        Err("Could not remove")
    }
